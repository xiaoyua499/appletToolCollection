		...
		<view class="contentBox">
			<checkbox-group @change="checkboxChange" class="content">
				<view v-for="(item ,index) in title" :key="index" class="checkbox-box">
					...
					<input type="text" class="titleInput" v-model="item.name" @blur="onChangeInput(item)" v-else>
				</view>
				<van-button round icon="plus" type="primary" size="mini" @click="addTitle" />
			</checkbox-group>
			...
			<view class="input-box">
				<textarea class="input" maxlength="-1" :placeholder='placeholder' v-model="str"></textarea>
...
<script setup>
	...
	const placeholder = '转换内容按上方勾选顺序依次填写,例:紫气东来:jiandan50180+126'
